...
namespace classicminimaps {
    ...
    vec2 getLatLng(){
        string result = executeCommand("/Users/adamc/Desktop/classicminimaps/assets/whereami ; exit;");
        if(result == ""){
            return longLat;
        }
	...
	void keyPresses(GLFWwindow* window, int key, int scancode, int action, int mods) {
        ...
        cout << to_string(action == GLFW_RELEASE) << endl;
		if (key == GLFW_KEY_R) {
